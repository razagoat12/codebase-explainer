...
async def _run_analysis(analysis_id: str, directory_path: str) -> None:
    """Background task: ingest directory → run Gemini pipeline → persist results."""
...
async def analyze_local(
    ...
    # Validate path upfront so we can return a 400 immediately if it's bad
    try:
    ...
    analysis = Analysis(user_id=current_user.id, directory_path=body.directory_path)
    db.add(analysis)
    await db.commit()
    await db.refresh(analysis)

    background_tasks.add_task(_run_analysis, analysis.id, body.directory_path)
...
async def get_history(
    ...
    return [
        AnalysisSummary(
            id=r.id, status=r.status, directory_path=r.directory_path, created_at=r.created_at
        )
        for r in rows
    ]
